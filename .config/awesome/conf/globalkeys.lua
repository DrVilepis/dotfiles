...
globalkeys = gears.table.join(
    ...
    awful.key({ }, "Print", 
	function (c) 
	    awful.util.spawn('/bin/sh -c "maim -us -n 1 | tee /tmp/$(date +%F_%T).png | xclip -selection clipboard -t image/png"', false)
    end,
    {description = "take screenshot of area", group = "utils"}),
    
    awful.key({ modkey }, "Print", 
	function (c) 
	    awful.util.spawn('/bin/sh -c "flameshot gui"', false) 
    end,
    ...
    function(c)
        awful.spawn("/bin/sh -c 'rofi -show window'", false);
    end,
    {description = "window focus prompt",group = "utils"}),

    awful.key({ modkey }, "i", 
    function()
        if(screen.primary.sidemenu.visible == true)
        then
            screen.primary.sidemenu.visible = false
        else
            screen.primary.sidemenu.visible = true
        end
    end)
)
